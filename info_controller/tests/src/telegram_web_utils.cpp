...
namespace gazeta::info_controller {
  namespace {
    ...
    void httpdoc_set_bubble(std::stringstream& httpdoc, int photos = 1, bool text_with_link = true) {
    ...
    bool parse_with_context(const std::string& httpdoc, std::function<bool(xmlXPathContextPtr context)> parse_method) {
      ...
      const bool inner_context_exists =
          info_sources::message_widgets_exists(context, tgme_widget_message_html_elements);
  ...
  void TestTelegramWeb::set_dataset_number(TestTelegramWebDataSets dataset_number) {
    ...
    case TestTelegramWebDataSets::var1: httpdoc = get_httpdoc_var1(); break;
    case TestTelegramWebDataSets::var2: httpdoc = get_httpdoc_var2(); break;
    case TestTelegramWebDataSets::var3: httpdoc = get_httpdoc_var3(); break;
    case TestTelegramWebDataSets::empty:
    default: httpdoc = get_httpdoc_empty(); break;
    }
  }
  ...
  std::vector<common::article> TestTelegramWeb::parse_all() {
    return parser->parse_doc(httpdoc);
  }

  bool TestTelegramWeb::parse_element_message_time(common::article& new_article) {
  ...
  bool TestTelegramWeb::parse_element_message_link(common::article& new_article) {
    return parse_with_context(httpdoc, [&new_article](xmlXPathContextPtr context) {
      return info_sources::parse_element_message_link(new_article, context);
    });
  }

  bool TestTelegramWeb::parse_element_message_text(common::article& new_article) {
    return parse_with_context(httpdoc, [&new_article](xmlXPathContextPtr context) {
      return info_sources::parse_element_message_text(new_article, context);
    });
  }

  bool TestTelegramWeb::parse_element_message_image(common::article& new_article) {
    return parse_with_context(httpdoc, [&new_article](xmlXPathContextPtr context) {
      return info_sources::parse_element_message_image(new_article, context);
    });
  }

  bool TestTelegramWeb::parse_element_message_video(common::article& new_article) {
    return parse_with_context(httpdoc, [&new_article](xmlXPathContextPtr context) {
      return info_sources::parse_element_message_video(new_article, context);
    });
  }

  bool TestTelegramWeb::parse_element_message_reply(common::article& new_article) {
    return parse_with_context(httpdoc, [&new_article](xmlXPathContextPtr context) {
